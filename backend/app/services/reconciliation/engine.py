...
class ReconciliationEngine:
    ...
    No AI is used here.
    This establishes the baseline against which
    future AI-assisted matching will be evaluated.
    """
    ...
    def _normalize_data(self) -> None:
        ...
        for column in [
            ...
            self.orders[column] = self.orders[column].astype(float)

        for column in [
            "payment_amount",
        ]:
            self.payments[column] = self.payments[column].astype(float)
        ...
        self.orders["order_id"] = (
            self.orders["order_id"].astype(str).str.strip()
        )

        self.payments["payment_id"] = (
            self.payments["payment_id"].astype(str).str.strip()
        )

        self.payments["order_id"] = (
            self.payments["order_id"].astype(str).str.strip()
        )

        self.settlements["payment_id"] = (
            self.settlements["payment_id"].astype(str).str.strip()
        )

    def _find_settlements(self, payment_id: str) -> pd.DataFrame:
        """Return all settlement records belonging to a payment."""

        return self.settlements[
            self.settlements["payment_id"] == payment_id
        ]

    def _find_order(self, order_id: str) -> pd.Series | None:
    ...
    def _validate_amount(
        ...
        """
        Validate payment against settlement gross amount.

        A tolerance prevents floating-point noise from being
        interpreted as a financial discrepancy.
        """
    ...
    def _check_settlement_delay(
        ...
        """Check whether settlement happened within expected time."""
    ...
    def reconcile(self) -> pd.DataFrame:
        ...
        for _, payment in self.payments.iterrows():
            ...
            settlements = self._find_settlements(payment_id)
            ...
            if len(settlements) > 1:
                ...
                settlement_ids = ", ".join(
                    settlements["settlement_id"].astype(str)
                )
